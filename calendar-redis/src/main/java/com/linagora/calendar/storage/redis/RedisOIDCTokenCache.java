...
public class RedisOIDCTokenCache implements OIDCTokenCache {
    ...
    private static final String TOKEN_CACHE_PREFIX = "tcal_oidc_hash_";
    private static final String SID_CACHE_PREFIX = "tcal_oidc_sid_";
    ...
    private final RedisTokenCacheCommands redisCommand;
    ...
    public RedisOIDCTokenCache(TokenInfoResolver tokenInfoResolver,
                               ...
                               RedisTokenCacheCommands redisCommand) {
    ...
    public Mono<Void> invalidate(Sid sid) {
        ...
        return redisCommand.lrange(sidRedisKey)
            .collectList()
            .filter(tokenRedisKeyList -> !tokenRedisKeyList.isEmpty())
            .flatMap(tokenRedisKeyList -> redisCommand.del(tokenRedisKeyList.toArray(String[]::new)))
            .then(redisCommand.del(sidRedisKey))
            .then()
            .onErrorResume(error -> {
    ...
    public Mono<TokenInfo> getTokenInfoFromCache(String tokenRedisKey) {
        return redisCommand.hgetall(tokenRedisKey)
    ...
    private Mono<Void> cacheAssociatedInformation(String tokenRedisKey, TokenInfo tokenInfo) {
        ...
        return redisCommand.hset(tokenRedisKey, mapData)
            .then(redisCommand.expire(tokenRedisKey, tokenCacheConfiguration.expiration()));
    }

    private Mono<String> cacheSidInfo(Sid sid, String tokenRedisKey) {
        return Mono.just(resolveSidRedisKey(sid))
            .flatMap(sidRedisKey -> redisCommand.rpush(sidRedisKey, tokenRedisKey)
                .then(redisCommand.expire(sidRedisKey, tokenCacheConfiguration.expiration()))

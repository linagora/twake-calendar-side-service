...
public class CollectedContactConverter {
    ...
    public Optional<ConvertedContact> convertForUpdate(byte[] existingVCard, ObjectNode contactData) {
        try {
            Card existingCard = toJSContact(existingVCard);
            Card incomingCard = Card.toJSCard(contactData.toString());
            ObjectNode existingContact = toObjectNode(existingCard);
            removeSourceVCardVersion(existingContact);
            ObjectNode contactToConvert = contactData;
            if (shouldMergeComplementaryIdentities(existingCard, incomingCard)) {
                contactToConvert = existingContact.deepCopy().setAll(contactData);
            }

            ConvertedContact convertedContact = convert(contactToConvert);
            ObjectNode updatedContact = toObjectNode(toJSContact(convertedContact.vcard()));
            removeSourceVCardVersion(updatedContact);
            if (existingContact.equals(updatedContact)) {
                return Optional.empty();
            }
            return Optional.of(convertedContact);
    ...
    private boolean shouldMergeComplementaryIdentities(Card existingCard, Card incomingCard) {
        ...
        return existingHasEmail && incomingHasMatrixId && !incomingHasEmail
            || existingHasMatrixId && !existingHasEmail && incomingHasEmail && !incomingHasMatrixId;
    }
    ...
    private ObjectNode toObjectNode(Card card) throws IOException {
        return OBJECT_MAPPER.readValue(Card.toJson(card), ObjectNode.class);
    }

    private void removeSourceVCardVersion(ObjectNode contact) {
        if (!(contact.get("vCardProps") instanceof ArrayNode vCardProps)) {
            return;
        }
        ...
        for (int index = vCardProps.size() - 1; index >= 0; index--) {
            if (vCardProps.get(index).isArray()
                && !vCardProps.get(index).isEmpty()
                && "version".equalsIgnoreCase(vCardProps.get(index).get(0).asText())) {
                vCardProps.remove(index);
            }
        }
        if (vCardProps.isEmpty()) {
            contact.remove("vCardProps");
        }
    }
